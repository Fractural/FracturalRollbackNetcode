...
namespace Game
{
    public class MonoMain : Node2D
    {
        ...
        private async void OnNetworkPeerConnected(int peerId)
        {
            ...
            if (SyncManager.Global.network_adaptor.IsNetworkHost())
        ...
        private void OnSyncManagerSyncStarted()
        {
            ...
            if (loggingEnabled && !SyncReplay.Global.active)
            {
                ...
                string logFileName = string.Format("{0}-{1}-{2}_{3}-{4}-{5}_peer-{6}.log", datetime["year"], datetime["month"], datetime["day"], datetime["hour"], datetime["minute"], datetime["second"], SyncManager.Global.network_adaptor.GetNetworkUniqueId());
        ...
        public void SetupMatchForReplay(int myPeerId, GDC.Array peerIds, GDC.Dictionary matchInfo)
        {
            connectionPanel.Visible = false;
            mainMenu.Visible = false;
        ...
        {
            SyncManager.Global.network_adaptor = new DummyNetworkAdaptor();
            SyncManager.Global.Start();

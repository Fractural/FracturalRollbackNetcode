...
public class FrameDataGraphCanvas : Control
{
	...
	public Array network_arrow_peers  = new Array(){};
	
	public bool show_rollback_ticks  = true;
	public int max_rollback_ticks  = 15;
	
	public static readonly Dictionary FRAME_TYPE_COLOR = new Dictionary(){
	...
	public void _DrawPeer(int peer_id, Rect2 peer_rect, Dictionary draw_data)
	{  
		...
		Array tick_numbers_to_draw  = new Array(){};
		
		bool capture_network_arrow_positions = show_network_arrows && peer_id in network_arrow_peers;
		Dictionary network_arrow_start_positions  = new Dictionary(){};
		Dictionary network_arrow_end_positions  = new Dictionary(){};
			...
			if(frame_rect.Intersects(extended_peer_rect))
			{
				...
				else if(skipped)
				{
					...
					if(frame.data.Has("skip_reason"))
					{
						...
						if(tick_letter != "")
						{
							tick_numbers_to_draw.Append(new Array(){_font_big, center_position - new Vector2(5, 0), tick_letter, new Color("f04dff")});
						}
					}
				}
				else
				...
				if(frame.type == Logger.FrameType.TICK && frame.data.Has("tick") && !skipped)
				{
					int tick = frame.data["tick"];
					tick_numbers_to_draw.Append(new Array(){_font, center_position - new Vector2(3, 0), GD.Str(tick), new Color(1.0, 1.0, 1.0)});
					if(frame.data.Has("input_tick") && capture_network_arrow_positions)
					{
				...
				if(capture_network_arrow_positions && frame.data.Has(other_network_arrow_peer_key))
				{
				...
				if(show_rollback_ticks && frame.data.Has("rollback_ticks"))
				{
		...
		if(capture_network_arrow_positions)
		{
			if(!draw_data.Has("network_arrow_positions"))
			{
				draw_data["network_arrow_positions"] = new Array(){};
			}
			draw_data["network_arrow_positions"].Append(new Array(){network_arrow_start_positions, network_arrow_end_positions});
	
		}
	}
	
	public void _DrawNetworkArrows(Dictionary start_positions, Dictionary end_positions, Color color)
	{  
		foreach(var tick in start_positions)
		{
			if(!end_positions.Has(tick))
			{
				continue;
			}
	...
	{  
		...
		Dictionary draw_data  = new Dictionary(){};
		Dictionary peer_rects  = new Dictionary(){};
		...
		if(show_network_arrows)
		{
			Array network_arrows_positions = draw_data.Get("network_arrow_positions", new Array(){});

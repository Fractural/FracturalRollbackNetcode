...
namespace Fractural.RollbackNetcode
{
    public class SyncManager : Node
    {
        ...
        public IDictionary<int, Peer> peers = new Dictionary<int, Peer>();
        public IList<InputBufferFrame> input_buffer = new List<InputBufferFrame>();
        public IList<StateBufferFrame> state_buffer = new List<StateBufferFrame>();
        public IList<StateHashFrame> state_hashes = new List<StateHashFrame>();
        ...
        public GDC.Dictionary _interpolation_state = new GDC.Dictionary() { };
        ...
        public GDC.Array _debug_check_local_state_consistency_buffer = new GDC.Array() { };
        ...
        public delegate void SyncErrorDelegate(GDC.Dictionary msg);
        ...
        public delegate void SceneSpawnedDelegate(string name, Node spawned_node, Node scene, GDC.Dictionary data);
        ...
        {
            foreach (string nodePathStr in _interpolation_state)
            {
                if (nodePathStr == "$")
                    continue;
                ...
                {
                    var states = _interpolation_state.Get<GDC.Dictionary>(nodePathStr);
                    interpolateState._InterpolateState(states.Get<GDC.Dictionary>(0), states.Get<GDC.Dictionary>(1), weight);
                }
            }
        }
        ...
        public void _SaveCurrentState()
        {
            ...
            state_buffer.Add(StateBufferFrame.new(current_tick, _CallSaveState()));
        ...
        public InputBufferFrame _PredictMissingInput(InputBufferFrame input_frame, InputBufferFrame previous_frame)
        {
            if (!input_frame.IsComplete(peers))
            {
                ...
                GDC.Dictionary missing_peers_predicted_input = new GDC.Dictionary() { };
                GDC.Dictionary missing_peers_ticks_since_real_input = new GDC.Dictionary() { };
                    ...
                    if (!node_master in missing_peers)
				{
                    ...
                    if (has_predict_network_input || previous_input.Contains(node_path_str))
                    {
                        ...
                        var predicted_input_for_node = has_predict_network_input ? Utils.CallInteropMethod(node, "_predict_remote_input", new GDC.Array() { previous_input_for_node, ticks_since_real_input }) : previous_input_for_node.Duplicate()
        ...
        public bool _CleanupBuffers()
        {
            ...
            while (state_buffer.Size() > max_buffer_size + 1)
            {
                ...
                if (!input_frame.IsComplete(peers))
                {
                    GDC.Array missing = input_frame.GetMissingPeers(peers);
        ...
        // only count input frames before the current frame towards the buffer size.
            ...
            while (state_hashes.Size() > (max_buffer_size * 2))
            {
                ...
                {
                    GDC.Array missing = state_hash_to_retire.GetMissingPeers(peers);
        ...
        public GDC.Dictionary GetCurrentInputFromPeerForPath(int peer_id, String path)
        {
            ...
            if (input_frame)
            {
                return input_frame.GetInputForPlayer(peer_id).Get(path, new GDC.Dictionary() { });
            }
            return new GDC.Dictionary() { };
        ...
        public GDC.Array _GetInputMessagesFromSendQueueInRange(int first_index, int last_index, bool reverse = false)
        {
            var indexes = !reverse ? GD.Range(first_index, last_index + 1) : GD.Range(last_index, first_index - 1, -1)



        GDC.Array all_messages = new GDC.Array() { };
        ...
        public GDC.Array _GetInputMessagesFromSendQueueForPeer(Peer peer)
        {
        ...
        public void _RecordAdvantage(bool force_calculate_advantage = false)
        {
            foreach (var peer in peers.Values())
            {
                ...
                if (_logger)
                {
                    _logger.AddValue(\"peer_%s\" % peer.peer_id, new GDC.Dictionary(){
...
public void _PhysicsProcess(float _delta)
{
    ...
    if (current_tick > 0)
    {
        ...
        if (interpolation)
        {
            ...
            GDC.Dictionary to_state = state_buffer[-1].data;
            GDC.Dictionary from_state = state_buffer[-2].data;
            _interpolation_state.Clear();
...
public void _OnReceivedInputTick(int peer_id, byte[] serialized_msg)
{
    ...
    GDC.Dictionary all_remote_input = msg[MessageSerializer.InputMessageKey.INPUT];
...
public Node Spawn(String name, Node parent, PackedScene scene, GDC.Dictionary data = new GDC.Dictionary() { }, bool rename = true, String signal_name = "")
{
    if (!started)
    {
...
public String OrderedDict2str(GDC.Dictionary dict)
{
    ...
    foreach (var i in dict.Size())
    {
        var key = dict.Keys()[i];
        var value = dict[key];
        var value_str = value is GDC.Dictionary ? OrderedDict2str(value) : GD.Str(value)
